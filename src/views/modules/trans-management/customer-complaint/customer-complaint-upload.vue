    ...
    data () {
      return {
        ...
        uploadConfig: {
          ...
          uploadUrl: '/transmanagement/tmcustomercomplaints/export', // 文件上传地址
    ...
    methods: {
      ...
      dataFormSubmit (val) {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl('/transmanagement/tmcustomercomplaints/trueExport'),
              method: 'post',
              ...
              if (data.type && data.type === '2') {
                ...
                if (data.code === 0) {
                  ...
                  this.$confirm(`本次客诉成功导入${data.feedBack.successNumber}条，失败${data.feedBack.falseNumber}条<br/>${exception}`, '导入成功！', {
                    confirmButtonText: '确定',
