<template>
  <el-dialog
    ...
    <el-form :model="dataForm" ref="dataForm" :rules="dataRule" @keyup.enter.native="dataFormSubmit()"
      ...
      <el-row>
        <el-col>
          <el-form-item label="上传扣罚单" prop="attachments">
  ...
  export default {
    ...
    data () {
      return {
        ...
        dataRule: {
          attachments: [{required: true, message: '请上传扣罚单', trigger: 'change'}]
        },
        uploadConfig: {
          ...
          uploadUrl: '/feesettlement/fsfine/export', // 文件上传地址
    ...
    methods: {
      ...
      dataFormSubmit (val) {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl('/feesettlement/fsfine/trueExport'),
              method: 'post',
              ...
              if (data.type && data.type === '2') {
                ...
                if (data.code === 0) {
                  ...
                  this.$confirm(`本次扣费单成功导入${data.feedBack.successNumber}条，失败${data.feedBack.falseNumber}条<br/>${exception}`, '导入成功！', {
                    confirmButtonText: '确定',
