      ...
      <el-row>
        <el-col :span="24" style="text-align: right;padding-right: 5px">
          <el-form-item style="margin-bottom: 5px">
            <el-button size="mini" icon="el-icon-circle-plus-outline" style="background: #3698ce;color: #ffffff" @click="addOrUpdateHandle()">
              新增</el-button>
            <el-button type="success" size="mini" :disabled="currentStatus === 1" @click="handleExamine()">
              <icon-svg name="审核" style="vertical-align: middle"></icon-svg>
              审核</el-button>
            <el-button type="success" size="mini" @click="handleExamineOrVoid(1)"
            ...
            <el-button type="danger" size="mini" @click="handleExamineOrVoid(2)"
                       >
              <icon-svg name="作废" style="vertical-align: middle"></icon-svg>
              作废</el-button>
            <el-button type="success" size="mini" @click="handleWriteOff()">
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            >
              ...
              编辑</el-button>
            <el-button type="warning" size="mini" @click="showDetails(scope.row.id)"
            >
              <icon-svg name="详情" style="vertical-align: middle"></icon-svg>
              详情
            </el-button>
            <!--<el-button type="success" size="mini" @click="handleExamine(scope)"
            <el-button v-if="scope.row.status === 0 || scope.row.status === 3" type="success" size="mini" @click="handleExamine(scope)"
            >
              <icon-svg name="审核" style="vertical-align: middle"></icon-svg>
              审核</el-button>-->
            <!--<el-button type="success" size="mini" @click="handleWriteOff(scope)"
            >
              <icon-svg name="付款核销" style="vertical-align: middle"></icon-svg>
              付款核销</el-button>-->
          </template>
  ...
  export default {
    ...
    methods: {
      ...
      handleExamineOrVoid (val) {
        ...
        this.$confirm(`承运商：${ids} 对账账期：${statementMonth}`, `确定${val === 1 ? '反审核' : val === 2 ? '作废' : ''}该对账单？`, {
      ...
      outPutExcel () {
        this.$http({
          url: this.$http.adornUrl('/feesettlement/tfsinvoice/exportFsInvoice'),
          method: 'post',
          data: this.$http.adornData({
            'invoiceNo': this.dataForm.invoiceNo,
            'carrierId': this.dataForm.carrierId,
            'status': this.dataForm.status,
            'invoiceMonth': this.dataForm.invoiceMonth && formatDate(this.dataForm.invoiceMonth, 'Month'),
            'begindate': this.dataForm.date && formatDate(this.dataForm.date[0]),
            'enddate': this.dataForm.date && formatDate(this.dataForm.date[1])
          }),
          responseType: 'blob'
        }).then(({data}) => {
