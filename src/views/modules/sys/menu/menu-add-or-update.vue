  ...
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="80px">
      ...
      <el-form-item :label="typeList[type] + '名称'" prop="name">
        <el-input v-model="dataForm.name" maxlength="16" :placeholder="typeList[type] + '名称'"></el-input>
      </el-form-item>
      ...
      <el-form-item v-if="type === 1" label="菜单路由" prop="url">
        <el-input v-model="dataForm.url" maxlength="200" placeholder="菜单路由"></el-input>
      </el-form-item>
      <el-form-item v-if="type !== 0" label="授权标识" prop="perms">
        <el-input v-model="dataForm.perms" maxlength="500" placeholder="多个用逗号分隔, 如: user:list,user:create"></el-input>
      </el-form-item>
      <el-form-item v-if="type !== 2" label="排序号" prop="orderNum">
        <el-input-number v-model="dataForm.orderNum" maxlength="8" controls-position="right" :min="0" label="排序号"></el-input-number>
      </el-form-item>
    ...
    data () {
      ...
      var validateName = (rule, value, callback) => {
        if (this.dataForm.name === '') {
          callback(new Error(`${this.typeList[this.type]}名称不能为空`))
        } else {
          callback()
        }
      }
      ...
      'type' () {
        if (this.isBegin) {
          this.$nextTick(() => {
            ...
            if (this.dataForm.id) {
              this.dataForm = Object.assign(dataForm, {id: this.dataForm.id})
            } else {
