<template>
  <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="100px">
    ...
    <el-row>
      <el-col :span="8">
        <el-form-item label="许可证号" prop="licenceNo">
          <el-input v-model="dataForm.licenceNo" maxlength="30" @blur="handleLicence" clearable placeholder="许可证号"></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="8">
    ...
    <el-row style="margin-top: 20px;text-align: right">
      <el-col>
        <el-form-item>
          <el-button @click="$router.push({name: 'trans-resources-business-license/business-license'})">取消</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleLicence (event) {
        this.$http({
          url: this.$http.adornUrl(`/transresources/trroadmanagementlicence/byLicenceNo/${event.target.value}`),
          method: 'get',
          params: this.$http.adornParams()
        }).then(({data}) => {
          if (data && data.code !== 0) {
            this.$message(data.msg)
            this.dataForm.licenceNo = ''
          }
        })
      }
    }
