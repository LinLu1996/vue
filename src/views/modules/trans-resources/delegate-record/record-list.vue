<template>
  <div class="mod-config">
    <el-form @keyup.enter.native="getDataList()" label-width="100px">
      <el-row>
        <el-col :span="8">
        ...
        <el-col :span="8">
          <el-form-item label="委托方">
            <xkl-select style="width: 100%" v-model="dataForm.consignorId" keyType="consignorId" labelType="ownerName" linkage="/transresources/ttrconsignor/list" placeholder="委托方"></xkl-select>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button v-if="scope.row.recordStatus !== 4 && scope.row.recordStatus !== 5" type="primary" size="mini" @click="recordHandle(scope.row.id)">
  ...
  export default {
    ...
    methods: {
      ...
      recordCommit (data) {
        ...
        this.$nextTick(() => {
          this.$refs.recordCommit.init(data)
        })
      },
      importSave () {
      ...
      toORBackExamine (id, status) {
        this.showCheckDetailsVisible = true
        this.$nextTick(() => {
          this.$refs.showCheckDetails.init(id, status)
        })
      },
